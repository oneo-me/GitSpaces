...
public class TextInlineRange
{
    ...
    public TextInlineRange(int p, int n)
    {
        Start = p;
        Count = n;
    }
...
public class TextDiffLine
{
    ...
    public List<TextInlineRange> Highlights { get; set; } = new();
...
public partial class TextDiff
{
    ...
    public List<TextDiffLine> Lines { get; set; } = new();
    ...
    public void GenerateNewPatchFromSelection(Change change, string fileBlobGuid, TextDiffSelection selection, bool revert, string output)
    {
        ...
        else
        {
            ...
            for (var i = selection.StartLine - 1; i < selection.EndLine; i++)
            {
                var line = Lines[i];
                if (line.Type != TextDiffLineType.Added) continue;
                builder.Append("\n+").Append(line.Content);
    ...
    public void GeneratePatchFromSelection(Change change, string fileTreeGuid, TextDiffSelection selection, bool revert, string output)
    {
        ...
        if (selection.EndLine < Lines.Count)
        {
            ...
            {
                for (var i = selection.EndLine; i < Lines.Count; i++)
                {
                    var line = Lines[i];
        ...
        if (Lines[selection.StartLine - 1].Type != TextDiffLineType.Indicator)
        {
            ...
            for (var i = selection.StartLine - 2; i >= 0; i--)
            {
                var line = Lines[i];
            ...
            for (var i = indicator; i < selection.StartLine - 1; i++)
            {
                var line = Lines[i];
                if (line.Type == TextDiffLineType.Indicator)
                {
    ...
    bool ProcessIndicatorForPatch(StringBuilder builder, TextDiffLine indicator, int idx, int start, int end, int ignoreRemoves, int ignoreAdds, bool revert, bool tailed)
    {
    ...
    bool ProcessIndicatorForPatchSingleSide(StringBuilder builder, TextDiffLine indicator, int idx, int start, int end, int ignoreRemoves, int ignoreAdds, bool revert, bool isOldSide, bool tailed)
    {
        var match = indicatorRegex().Match(indicator.Content);
        var oldStart = int.Parse(match.Groups[1].Value);
        var newStart = int.Parse(match.Groups[2].Value) + ignoreRemoves - ignoreAdds;
...
{
    public LFSObject Old { get; set; } = new();
    public LFSObject New { get; set; } = new();
}
