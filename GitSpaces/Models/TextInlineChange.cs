...
public class TextInlineChange
{
    ...
    static List<Chunk> MakeChunks(Dictionary<string, int> hashes, string text)
    {
        var start = 0;
    ...
    static void CheckModified(List<Chunk> chunksOld, int startOld, int endOld, List<Chunk> chunksNew, int startNew, int endNew, int[] forward, int[] reverse)
    {
        ...
        {
            for (var i = startOld; i < endOld; i++) chunksOld[i].Modified = true;
        }
        else if (lenNew > 0)
        {
            for (var i = startNew; i < endNew; i++) chunksNew[i].Modified = true;
        }
    }

    static EditResult CheckModifiedEdit(List<Chunk> chunksOld, int startOld, int endOld, List<Chunk> chunksNew, int startNew, int endNew, int[] forward, int[] reverse)
    {
        var lenOld = endOld - startOld;
        var lenNew = endNew - startNew;
    ...
    static void AddChunk(List<Chunk> chunks, Dictionary<string, int> hashes, string data, int start)
    {
        int hash;
        if (hashes.TryGetValue(data, out hash))
        {
            chunks.Add(new(hash, start, data.Length));
        }
        else
        {
            hash = hashes.Count;
