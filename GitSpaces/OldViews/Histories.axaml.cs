    ...
    void RefreshLayout()
    {
        if (UseHorizontal)
        {
            ...
            for (var i = 0; i < Children.Count; i++)
            {
                ...
                if (child is GridSplitter splitter) splitter.BorderThickness = new(1, 0, 0, 0);
            }
        }
        else
        {
            ...
            for (var i = 0; i < Children.Count; i++)
            {
                ...
                if (child is GridSplitter splitter) splitter.BorderThickness = new(0, 1, 0, 0);
            }
        }
    }
}
...
public class CommitGraph : Control
{
    ...
    [
        new(Brushes.Orange, 2),
        new(Brushes.ForestGreen, 2),
        new(Brushes.Gold, 2),
        new(Brushes.Magenta, 2),
        new(Brushes.Red, 2),
        new(Brushes.Gray, 2),
        new(Brushes.Turquoise, 2),
        new(Brushes.Olive, 2)
    ];
    ...
    public override void Render(DrawingContext context)
    {
        ...
        // Apply scroll offset.
        context.PushClip(new(Bounds.Left, Bounds.Top, grid.Columns[0].ActualWidth, Bounds.Height));
        ...
        // Draw connect dots
        Brush dotFill = null;
        if (Application.Current.TryGetResource("Brush.Contents", Application.Current.ActualThemeVariant, out var res) && res is SolidColorBrush)
        {
    ...
    void DrawCurves(DrawingContext context, double top, double bottom)
    {
        foreach (var line in Graph.Paths)
        {
            ...
            using (var ctx = geo.Open())
            {
                ...
                for (var i = 1; i < size; i++)
                {
                    ...
                    if (cur.Y > bottom)
                    {
                        cur = new(cur.X, bottom);
                        ended = true;
                    }

                    if (cur.X > last.X)
                    {
                        ctx.QuadraticBezierTo(new(cur.X, last.Y), cur);
                    }
                    else if (cur.X < last.X)
                    {
                        if (i < size - 1)
                        {
                            ...
                            ctx.QuadraticBezierTo(new(last.X, midY), new(midX, midY));
                            ctx.QuadraticBezierTo(new(cur.X, midY), cur);
                        }
                        else
                        {
                            ctx.QuadraticBezierTo(new(last.X, cur.Y), cur);
                        }
                    }
                    else
                    {
...
public partial class Histories : UserControl
{
    ...
    void OnCommitDataGridLayoutUpdated(object sender, EventArgs e)
    {
        commitGraph.InvalidateVisual();
    }

    void OnCommitDataGridSelectionChanged(object sender, SelectionChangedEventArgs e)
    {
    ...
    void OnCommitDataGridContextRequested(object sender, ContextRequestedEventArgs e)
    {
        if (DataContext is ViewModels.Histories histories)
        {
